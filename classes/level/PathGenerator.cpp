...
CoordinateList CreateFixedPath(CoordinateList* pPath) {
	CoordinateList FixedPath;
	
	if(!pPath) return FixedPath;
	if(pPath->empty()) return FixedPath;
	
	NVector2 Delta, PrevDelta;
    PrevDelta = NVector2(abs(pPath->at(1).x - pPath->at(0).x), abs(pPath->at(1).y - pPath->at(0).y));
    for(int i=1; i<pPath->size(); i++) {
    	Delta = NVector2(abs(pPath->at(i).x - pPath->at(i-1).x), abs(pPath->at(i).y - pPath->at(i-1).y));
		if(Delta != PrevDelta) {
			FixedPath.push_back(pPath->at(i-1));
		}
		PrevDelta = Delta;
	}
	FixedPath.push_back(pPath->back());
