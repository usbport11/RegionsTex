...
bool MAtlasBuffer::Initialize(stTexture* inpTexture, unsigned int TexSizeX, unsigned int TexSizeY, unsigned int TilesCountX, unsigned int TilesCountY)
{
	...
	if(!IsPowerOfTwo(TexSizeX) || !IsPowerOfTwo(TexSizeY)) {
		LogFile<<"Atlas buffer: Texture size not power of 2"<<endl;
		return false;
	}
	if(!IsPowerOfTwo(TexSizeX / TilesCountX) || !IsPowerOfTwo(TexSizeY / TilesCountY)) {
		LogFile<<"Atlas buffer: Tile size not power of 2: "<<endl;
		return false;
	}
...
bool MAtlasBuffer::AddFixedData(glm::vec2 vLeftBottom, glm::vec2 vRightTop, unsigned int TileNumberX, unsigned int TileNumberY) {
	//4 vecticies
	Verticies.push_back(glm::vec2(vLeftBottom.x, vLeftBottom.y));
	Verticies.push_back(glm::vec2(vRightTop.x, vLeftBottom.y));
	Verticies.push_back(glm::vec2(vRightTop.x, vRightTop.y));
	...
	TexCoords[0] = glm::vec2(TileNumberX * TileSize.x + HalfPixelOffset.x, TileNumberY * TileSize.y + HalfPixelOffset.y);
	TexCoords[1] = glm::vec2(TileNumberX * TileSize.x + TileSize.x - HalfPixelOffset.x, TileNumberY * TileSize.y + HalfPixelOffset.y);
	TexCoords[2] = glm::vec2(TileNumberX * TileSize.x + TileSize.x - HalfPixelOffset.x, TileNumberY * TileSize.y + TileSize.y - HalfPixelOffset.y);
	TexCoords[3] = glm::vec2(TileNumberX * TileSize.x + HalfPixelOffset.x, TileNumberY * TileSize.y + TileSize.y - HalfPixelOffset.y);
